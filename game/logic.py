...
class Logic:
            ...
            for obj in sorted(self.sprites.list_of_objects, key=lambda obj: obj.distance_to_sprite):
                if obj.is_on_fire[1]:
                    if obj.is_dead != 'immortal' and not obj.is_dead:
                        if ray_casting_enemy_player(obj.x, obj.y,
    ...
    def check_win(self):
        if not len([obj for obj in self.sprites.list_of_objects if obj.flag == 'enemy' and not obj.is_dead]):
...
def ray_casting_enemy_player(enemy_x, enemy_y, blocked_doors, world_map, player_position):
